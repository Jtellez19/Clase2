...
hoja = pd.read_csv('C:\\Users\\JTELLEZ\\Downloads\\clean_students_complete.csv')
escuelaGrado = hoja.groupby(["school_name", "grade"], as_index=False).mean().sort_values(["school_name", "grade"])
mejores = escuelaGrado[escuelaGrado.groupby(["school_name"])["reading_score"].transform(max) == escuelaGrado["reading_score"]]
